...
//  required_version = ">= 0.14.5"
  ...
  backend "remote" {
    ...
    workspaces {
      name = "terraform-vsphere--kubernetes-svndk8s06"
    }
  }
}
provider "vsphere" {
...
# 1 easyrsa; 8GB
# 3 etcd; 8GB
# 1 kubeapi-load-balancer; mem=4G root-disk=8G
# 2 kubernetes-master; cores=2 mem=4G root-disk=16G
# 3 kubernetes-worker; cores=4 mem=4G root-disk=16G
###
...
resource "vsphere_virtual_machine" "vm" {
  ...
  datastore_id             = data.vsphere_datastore.cluster.id
